...
object OrderApp {
  def main(args: Array[String]): Unit = {
      ...
      val sourceDStream: DStream[String] = MyKafkaUtil.getKafkaStream(ssc, Constant.TOPIC_ORDER)
      sourceDStream.print(50)

      val orerInfoDStream: DStream[OrderInfo] = sourceDStream.map { // 对数据格式做调整
        case x => {
          val orderInfo = JSON.parseObject(x, classOf[OrderInfo]) // 李小名 => 李**
